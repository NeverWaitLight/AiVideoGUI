...
-   [Image-to-video (first frame) API reference](/help/en/model-studio/legacy-image-to-video-api-reference/)
    
-   [Image-to-video (first and last frames) API reference](/help/en/model-studio/legacy-image-to-video-by-first-and-last-frame-api-reference)
...
A prompt describes what appears and moves in a video.

## **Basic formula**

Simple prompts work well for creative exploration.

| **Prompt = Entity + Scene + Motion** **Entity:** The main subject of the video. It can be a person, animal, plant, object, or an imaginary object. **Scene:** The environment where the entity is located. It includes the background and foreground and can be a real or imaginary setting. **Motion:** The specific movement of the entity and other elements in the scene. This can range from a still scene to slight, large, local, or overall movement. |   |   |
| --- | --- | --- |

## **Advanced formula**

Expand the basic formula for better quality, vividness, and storytelling. Most weak prompts cover entity and motion but skip aesthetic control and stylization — producing a static camera in an undefined void. Cover all five dimensions.

| **Prompt = Entity (description) + Scene (description) + Motion (description) + Aesthetic control + Stylization** **Entity description:** Describes the details of the entity's appearance. List adjectives or short phrases. For example, "a young woman with wavy auburn hair in a vintage floral dress" or "a celestial fairy from another world, dressed in tattered yet magnificent clothes, with a pair of strange wings made of debris from ruins on her back." **Scene description:** Describes the details of the entity's environment. List adjectives or short phrases. **Motion description:** Describes the details of the motion, including its amplitude, speed, and effect. For example, "swaying violently," "moving slowly," or "shattering the glass." **Aesthetic control:** Includes light source, lighting environment, shot size, camera angle, lens, and camera movement. For common cinematic terms, see the prompt dictionary below. Each camera move carries an emotional intent: a push-in creates intimacy or tension, a pull-out reveals scale or isolation, a tracking shot places the viewer alongside the subject, an orbit emphasizes the subject's importance, and a fixed camera signals stillness and focus. **Stylization:** Describes the visual style of the video. For example, "cyberpunk," "line art illustration," or "wasteland style." For common styles, see the prompt dictionary below. |   |   |
| --- | --- | --- |

## **Image-to-video formula**

The image defines the entity, scene, and style. Your prompt should focus on motion and camera movements.

| **Prompt = Motion + Camera movement** **Motion description:** Describe the movement of elements in the image, such as people running or animals waving. Use adjectives such as "quickly" or "slowly" to control the speed and intensity of the motion. **Camera movement:** For specific camera movement, use prompts such as "camera pushes in" or "camera moves left." To keep the camera still, use "fixed camera." |   |   |
| --- | --- | --- |

## **Sound formula (Wan 2.7/2.6/2.5)**

Add voice, sound effect, and background music descriptions to control audio.

| **Prompt = Entity + Scene + Motion + Sound description (voice/sound effect/background music)** **Sound description:** Describes the sound elements in the video and their changes over time. It guides the sound content and atmosphere to match the visual narrative. The three main categories are human voice, sound effects, and background music (BGM). |   |   |
| --- | --- | --- |
| **Voice = Character's lines + Emotion + Tone + Speed + Timbre + Accent** > Example: A man is doing a stand-up comedy routine. He says, **"Study hard and make progress every day,"** in a relaxed tone, at a moderate speed, with a clear voice, in American English. **Sound effect = Source material + Action + Ambient sound** > Example: A small glass ball falls from a table onto a wooden floor, making a "thud" sound in a quiet indoor environment. **Background music = Background music/score + Style** > Example: On a rainy night, a spooky, narrow corridor has a window at the end, with suspenseful background music. |   |   |

## **Multi-shot formula (Wan 2.7/2.6)**

Generate coherent multi-shot narratives with control over structure, camera position, and duration. Key elements stay consistent across shots.

| **Prompt = Overall description + Shot number + Timestamp + Shot content** **Overall description:** Briefly summarize the entire video. State the story's theme, narrative style, main emotion, or core event. This helps the model grasp the overall narrative direction. **Shot number:** Assign a number to each shot to define the sequence of scenes or segments. This organizes the video structure. For example, 'Shot 1 xxxx, Shot 2 xxxx' or 'Shot 1 \\[0–3 s\\] xxxx, Shot 2 \\[3–6 s\\] xxx'. **Timestamp:** Specify the time range for each shot. This ties shot content to the video timeline and improves generation accuracy. **Shot content:** Describe the specific behavior of the main character or object in each shot. Include actions, speech, expressions, and posture. Follow the general prompt writing style for a single shot. |   |   |
| --- | --- | --- |
| **Example:** This story is a short play about giving up and regaining hope, told from a third-person perspective. Shot 1 \\[0–3 s\\] A boy sits alone in a corner of a playground, looking down at a letter in his hand. He sighs softly, his eyes filled with confusion. Shot 2 \\[4–6 s\\] Hard cut transition, fixed camera position, focusing on the boy's eyes, which are glistening with tears, showing loss and helplessness. Shot 3 \\[7–10 s\\] Hard cut transition, the scene changes to a simple classroom. A girl with a gentle and firm gaze, dressed in plain clothes, walks over to the boy with a warm and determined smile to comfort him. |   |   |

**Single-shot note:** Wan 2.7 no longer supports the `shot_type` parameter. The model interprets the prompt directly. To generate a single-shot video, write 'Generate single shot.' in English or 'generate a single shot' in Chinese.

## **Reference-to-video formula (Wan 2.7/2.6)**

Generate videos that reference uploaded images or videos. The model preserves appearance, motion, voice timbre (if present), and background. Supported subjects: people, cartoons, pets, and props.

## **Wan 2.7**

| **Prompt = Reference identifier + Action + Scene + Lines (optional) + Background music (optional)** **Reference identifier:** Use 'Image n' or 'Video n' to refer to uploaded files, where n is the file's index within its type. Images and videos are numbered separately. > **Reference type + index:** For images, write 'Image n'. For videos, write 'Video n'. If you upload three images, they are 'Image 1', 'Image 2', and 'Image 3'. Index numbers match the upload order. Images and videos are counted separately. If only one reference is used, you can omit the number—for example, 'reference image' or 'reference video'. > **Chinese and English format:** For Chinese, use "Image1" and "Video1". For English, use "Image 1" and "Video 1". In the English format, capitalize the first letter and add a space between the word and the number. > **Describing visual content:** Suppose Image 1 shows a cat and Image 2 shows a room. To describe the cat playing in the room, you can use either: ① Direct identifiers, for example, 'Image 1 in Image 2'. or ② Identifier plus object/scene, for example, 'the cat in Image 1 plays in the room in Image 2'. **Action:** Describe the motion state of the subject or other elements. This includes static states, changes in expression and emotion, body movements, external actions, and displacement. **Scene:** The environment where the subject exists. It includes the background and foreground and can be a real or imaginary setting. **Lines:** The subject's spoken content. When you describe lines in the prompt—for example, 'X says: 'xxx''—the model preserves them exactly. If you do not describe lines, the model adds them freely. To suppress lines entirely, write 'No dialogue.' in English or 'no dialogue' in Chinese. **Background music:** When you omit background music in the prompt, the model selects it based on context and content. To suppress background music, write 'No background music.' in English or 'no background music' in Chinese. |
| --- |
| **Example 1:** Video 2 holds Image 3 while playing a soothing American folk song in a café. Video 1 smiles at Video 2. > In this example, Video 1 references a female, Video 2 references a male, and Image 3 references a guitar. |

## **Wan 2.6**

| **Prompt = Character + Action + Lines + Scene** **Character:** Reference the main character using an identifier such as 'character1'. Reference up to three characters at the same time. Each character can be referenced multiple times in the prompt to control their behavior. **Action:** Describe the motion state of the character or other elements. This includes static states, changes in expression and emotion, body movements, external actions, and displacement. **Lines:** The character's spoken content. This supports a single character speaking or a dialogue between multiple characters. **Scene:** The environment where the character exists. It includes the background and foreground and can be a real or imaginary setting. |   |   |
| --- | --- | --- |
| **Example:** This is a whimsical fairytale scene. character1 jumps and plays on the grass. character2 plays piano under a nearby apple tree. An apple falls on character2's head. character1 points at character2 and says happily, "You're going to become a scientist!". > In this example, the reference video for character1 is a rabbit, and the reference video for character2 is a dog. Each reference video contains only a single character. |   |   |

## Prompt optimization techniques

##### 1\. Automatic optimization: prompt\_extend

Enable the `prompt_extend` parameter in the API call. The system expands short prompts (e.g., "A kitten plays with a snowball in the snow") into detailed, structured descriptions.

##### 2\. Generate prompts using prompt formulas

For precise control over details, style, or complex dynamics, write prompts using structured formulas.

Alternatively, pass formulas as system messages to a model like qwen3.6-plus to generate prompts.

**Example: Use the sound formula to optimize a prompt**

> For more code examples, see [Overview](/help/en/model-studio/text-generation) .
...
##### 3\. What not to prompt

Avoid these patterns — they produce poor results:

| What to avoid | Why it fails |
| --- | --- |
| Specific real people by name | Most platforms reject these or produce inconsistent results |
| Rapid scene changes within a single clip | Cuts happen between clips, not within them. One clip = one continuous shot |
| Exact text legibility | Text renders approximately — specific words will not be precise |
| Very long or complex action sequences | A character performing a 30-second choreographed sequence. Keep actions simple and short |
| Lip-synced dialogue to specific words | Lip sync to exact words does not work reliably |

## Sound generation

Describe human voices, ambient sounds, and other audio elements to generate videos with synchronized audio.
...
| **Singing** | Surreal style, medium full shot, daylight, clear sky light, soft lighting, side lighting, warm tones, center composition. Two young Caucasian female models are situated in a lush, rolling green grassland, set against a backdrop of a pure blue sky adorned with fluffy, picturesque white clouds. In the foreground, a woman in her twenties, with fair, freckled skin, wears a sky-blue knitted vest. Her slightly curly brown hair is wrapped in a light blue headscarf. She lies on her side on the soft grass, her right hand gently supporting her cheek, and her left arm naturally extended. Her clear eyes look directly at the camera, a slight smile playing on her lips. A gentle breeze brushes her hair and clothing. Behind her, another young woman leans against a giant, cartoon-style pink mushroom with white dots. A burgundy crocheted headband encircles her hair, and she wears a yellow camisole paired with a flowing white long skirt, its hem gently swaying in the wind. Her eyes are closed, her expression serene and content, as if deeply immersed in a sunlit dream. The mushroom's surface is smooth and saturated in color, resembling a prop from a fairy tale. **The scene is accompanied by gentle folk guitar music. The foreground woman softly hums: "Follow the light, where the wild flowers grow," her voice clear and soft. In the background, bird calls and the distant rustling of leaves filter through, adding to the sense of natural tranquility.** |     |

**Multi-character dialogue: four naming principles**

When two or more characters speak in the same clip, the model needs clear signals to avoid merging or misattributing speech. Follow these principles:

| Principle | Guideline | Correct | Incorrect |
| --- | --- | --- | --- |
| Unique labels | Character labels must be unique and consistent — no pronouns or synonyms | `[Character A: Black-suited Agent]` and `[Character B: Female Assistant]` | `[Agent] says... Then, he says...` |
| Visual anchoring | Bind dialogue to a character's unique action — describe the action first, then the speech | `The agent slams his hand on the table. [Black-suited Agent, angrily]: "Where is the truth?"` | `[Black-suited Agent]: "Where is the truth?"` (no action anchor) |
| Distinct voice labels | Assign a unique tone and emotion label to each character | `[Black-suited Agent, raspy deep voice]: "Don't move."` | `[Man] says...` (too vague) |
| Temporal linking | Use clear linking words to control sequence and rhythm | `[Agent]: "Why?" Immediately, [Assistant]: "Because it's time."` | `[Agent]: "Why?" [Assistant]: "Because it's time."` (may merge speech) |
...
Add cinematic dimensions to control visual style and expressiveness.
